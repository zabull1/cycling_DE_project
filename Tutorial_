...
Step 1: Setup
Please see GCP Setup  if you need to set up a GCP Environment, including a VM, GCP account with the service account and project.
Clone on your VM, `git clone https://github.com/zabull1/cycling_DE_project.git`
Change the directory to the repo folder using `cd`
To install Google Cloud CLI, Terraform and Anaconda run `bash ./setup/setup.sh` 
Setup virtual environment by running the below commands
		source ~/.bashrc
		conda create -n mpls311 python=3.9 -y
		conda activate mpls311
		pip install -r ./setup/conda_requirements.txt
Save your Google Cloud Account .json file to the .credentials folder.
